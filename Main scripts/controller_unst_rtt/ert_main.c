...
void rt_OneStep(void)
{
  ...
  controller_unst_output();

  /* Get model outputs here */
  controller_unst_update();
...
int_T main(void)
{
  ...
  digitalWrite(13, 0);
  interrupts();
  controller_unst_initialize();
  Setup_TickTimer(TC_FOR_TIMER8, CHANNEL_FOR_TIMER8, ID_TC_FOR_TIMER8,
                  IRQn_FOR_TIMER8, TC_CMR_TCCLKS_TIMER_CLOCK3, 26250U);
  ...
  /* The main step loop */
  while (rtmGetErrorStatus(controller_unst_M) == (NULL)) {
    /* Check for overrun */
    if (IsrOverrun) {
      digitalWrite(13, 1);
